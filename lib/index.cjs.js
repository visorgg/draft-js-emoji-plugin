...
var clearEmojiList = function clearEmojiList() {
  Object.keys(MAYHEM_EMOJIONE).forEach(function (key) {
    delete object[key];
  });
};
...
var Emoji = function Emoji(_ref) {
  ...
  var shortName = MAYHEM_EMOJIONE.toShort(decoratedText);
  var emojiDisplay = null;

  if (useNativeArt === true) {
    emojiDisplay = /*#__PURE__*/React__default.createElement("span", {
      title: MAYHEM_EMOJIONE.toShort(decoratedText)
    }, props.children);
  } else {
    // short name to image url code steal from emojione source code
    var emojiObj = MAYHEM_EMOJIONE.emojiList[shortName];
    var shortNameForImage = MAYHEM_EMOJIONE.getShortnameFromEmoji(emojiObj);
...
var newEmojiListWithOutPriorityList = function newEmojiListWithOutPriorityList(priorityList) {
  ...
  for (var key in MAYHEM_EMOJIONE.emojiList) {
    ...
    list[key] = MAYHEM_EMOJIONE.emojiList[key].unicode;
  }
  ...
  _createClass(Entry, [{
    ...
    value: function render() {
      ...
      if (useNativeArt === true) {
        ...
        // short name to image url code steal from emojione source code
        var emojiObj = MAYHEM_EMOJIONE.emojiList[this.props.emoji];
        var shortNameForImage = MAYHEM_EMOJIONE.getShortnameFromEmoji(emojiObj);
...
var unicodeRegex = new RegExp(MAYHEM_EMOJIONE.unicodeRegexp, 'g');
...
var unicodeRegex$1 = new RegExp(MAYHEM_EMOJIONE.unicodeRegexp, 'g');
/*
...
var index = (function () {
  ...
  var cacheBustParam = allowImageCache ? '' : defaultCacheBustParam; // Adjust the plugin to accept an emoji list to merge.
  ...
  if (priorityList) emojiList.setPriorityList(priorityList); // Expose some of this plugins state to the invoker so that they can control some core pieces
